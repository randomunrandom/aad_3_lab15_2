...
int how_many_2s(int num) {
    ...
    for(int rank = 0; rank < len; ++rank) {
        ...
        if      (digit < 2)   {count += (num - (num % next_pow))/10;}                       //т.к. цифра меньше 2, она добавляет уоличество двоек как число с цифрой справа на той же позиции, все остальные 0
        else if (digit == 2)  {count += (num - (num % next_pow))/10 + (num % cur_pow) + 1;} //так же как вышеб только добавляется остаток справа +1б чтобы считать 0
        else if (digit > 2)   {count += (num - (num % next_pow))/10 + next_pow;}            //т.к. цифра меньше 2, она добавляет уоличество двоек как число с цифрой справа + 1 на той же позиции, все остальные 0
    }
    return count;
}